...
import by.verkpavel.grafolnet.parser.Segmenter
...
class ParserActor extends Actor {
  ...
  def receive = {
    case id: String =>
      ...
      segmenter.loadImage(image)
      val lines = segmenter.getLines()
      ...
      val words = segmenter.getWords();
      val symbols = segmenter.getSymbols();
