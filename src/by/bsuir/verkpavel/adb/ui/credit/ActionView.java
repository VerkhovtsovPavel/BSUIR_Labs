...
public abstract class ActionView extends JFrame {
    ...
    protected void fillFields(Credit credit) throws ParseException {
        ...
        creditPeriodTextField.setText(""
                + ChronoUnit.DAYS.between(startDateModel.getValue(), endDateModel.getValue()));
        ...
        if (checkRequiredFields(contractNumber)) {
            if (startDate.isBefore(LocalDate.now()) || endDate.isBefore(startDate)) {
                JOptionPane.showMessageDialog(null,
                        Messages.STARTDATEBEFORENOW_OR_ENDDATEBEFORESTART.get(), "Error",
                        JOptionPane.PLAIN_MESSAGE);
                return null;
            }
            if (startDate.isAfter(maxDate) || endDate.isAfter(maxDate)) {
                JOptionPane.showMessageDialog(null, Messages.DATE_AFTER_MAX_DATE.get(), "Error",
                        JOptionPane.PLAIN_MESSAGE);
                return null;
            }
            return new Credit(contractNumber, depositType, startDate.format(dateMask),
                    endDate.format(dateMask), currency, persent, depositSum, client);
        } else {
    ...
    private void createActionElements() throws ParseException {
        ...
        startDateField = new JDatePickerImpl(datePanel, new DateLabelFormatter());
        ...
        creditTypeComboBox.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                case "Кредит с ежемесячным погашением долга аннуитетным платежом":
                    startDateModel.removeChangeListener(dayRangeDateChangeListener);
                    endDateModel.removeChangeListener(dayRangeDateChangeListener);
                    startDateModel.addChangeListener(monthRangeDateChangeListener);
                    endDateModel.addChangeListener(monthRangeDateChangeListener);
                    break;
                case "Кредит с ежемесячным погашением процентов по кредиту и выплатой полной cуммы кредита в конце срока.":
                    startDateModel.removeChangeListener(monthRangeDateChangeListener);
                    endDateModel.removeChangeListener(monthRangeDateChangeListener);
                    startDateModel.addChangeListener(dayRangeDateChangeListener);
        ...
        currencyComboBox.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                creditSumField.setFormatterFactory(new DefaultFormatterFactory(formatter));
            }
        });
    ...
    private void fillComboBoxes() {
        fillComboBox(creditTypeComboBox, CreditProvider.getInstance().getCreditTypeList());
        fillComboBox(currencyComboBox, DepositProvider.getInstance().getCurrency());
        fillComboBox(clientComboBox, ClientProvider.getInstance().getUserFullNames());
    }
    ...
    public class DayRangeDateChangeListener implements ChangeListener {
        ...
        public void stateChanged(ChangeEvent e) {
            ...
            creditPeriodTextField.setText("" + ChronoUnit.DAYS.between(startDate, endDate));
        }
    }
    ...
    public class MonthRangeDateChangeListener implements ChangeListener {
        ...
        public void stateChanged(ChangeEvent e) {
            ...
            if (!endDate.isAfter(startDate)
                ...
                long daysValue = 30 * (((int) ChronoUnit.DAYS.between(startDate, endDate) + 2) / 30);
