	...
	public ArrayList<Recipe> convertCursorToArrayList(DBCursor cursor) {
		...
		try {
			while (cursor.hasNext()) {
				...
				Recipe recipe = new Recipe((String) currentObject.get("name"), (Integer) currentObject.get("time required"),
						(ArrayList<String>) currentObject.get("ingredients"), (String) currentObject.get("recipe"));
				result.add(recipe);
			}
		} finally {
			cursor.close();
