...
class FactoryParametersParser {
	...
	private static final String FRUIT_REGEXP = String.format("fruit%s\\w+", SPACE_REGEXP);
	private static final String SWEETNESS_REGEXP = String.format("sweetness%s\\d+", SPACE_REGEXP);
	...
	private static HashMap<String, Object> getCandiedRoastedNutsParams(String request) {
	...
	private static HashMap<String, Object> getSweetParams(String request) {
		...
		parameters.put("Sweetness", getSweetness(request));
		parameters.put("Cost", getCost(request));

		return parameters;
	}

	private static Object getSweetness(String request) {
		Pattern functionPattern = Pattern.compile(SWEETNESS_REGEXP);
		Matcher matcher = functionPattern.matcher(request);
		matcher.find();
		Integer sweetness = Integer.parseInt(matcher.group().split(SPACE_REGEXP)[1]);
		return sweetness;
	}
