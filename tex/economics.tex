...
% ---
Целью данного проекта является разработка и анализ алгоритмов определения психологических характеристик по образцам почерка, автоматизация графологических методов и удешевление экспертизы.
...
Расчеты выполнены на основе методического пособия~\cite{gorovoi_2014}.
...
\FPeval{baseSalary}{round(\workingHoursInDay * \bonusRate * (\hourlySalarySenior * \workloadSenior + \hourlySalaryLead * \workloadLead), 0)}
...
\FPeval{mountSalarySenior}{round(\workingHoursInMonth * \bonusRate * \hourlySalarySenior, 2)}
\FPeval{mountSalaryLead}{round(\workingHoursInMonth * \bonusRate * \hourlySalaryLead, 2)}
\FPeval{baseSalarySenior}{round(\workingHoursInDay * \bonusRate * \hourlySalarySenior * \workloadSenior, 2)}
\FPeval{baseSalaryLead}{round(\workingHoursInDay * \bonusRate * \hourlySalaryLead * \workloadLead, 2)}
\FPeval{hourlyWorkloadSenior}{round(\workloadSenior * 8, 0)}
\FPeval{hourlyWorkloadLead}{round(\workloadLead * 8, 0)}
...
\begin{table}[ht]
  ...
  \begin{tabular}{| >{\centering}m{0.03\textwidth}
...
%Налог на прибыль
\FPeval{profitTax}{round(\profitMoney * \profitTaxRate / 100, 0)}
...
\FPeval{baseSalaryAnalytic}{round(\workingHoursInDay * \bonusRate * (\hourlySalaryAnalytic * \workingDaysInYear), 0)}

% Дополнительная зарплата, Зд
\FPeval{additionalSalaryAnalytic}{round(\baseSalaryAnalytic * \additionalSalaryRate / 100, 0)}
...
% Отчисления в фонд социальной защиты
\FPeval{socialProtectionMoneyAnalytic}{round(\totalSalary * \socialProtectionRate / 100, 0)}

% Отчисления в белгосстрах
\FPeval{bgsMoneyAnalytic}{round(\totalSalary * \bgsRate / 100, 0)}

% Расходы на научные командировки
\FPeval{businessTripAnalyticMoney}{ round( \baseSalaryAnalytic * \businessTripRate / 100, 0 ) }

% Общая сумма
\FPeval{\totalAnalyticExpense}{round(\totalSalaryAnalytic + \socialProtectionMoney + \bgsMoneyAnalytic + \businessTripAnalyticMoney, 0)}
...
\FPeval{benefitPeriod}{ round(\sellingPrice / (\totalAnalyticExpense * 2), 2 ) }
\begin{equation}
