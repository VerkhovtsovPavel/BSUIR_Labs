...
\begin{longtable}[ht]{| >{\raggedright}m{0.24\textwidth}
...
Программное средство определение психологических параметров человека по анализу образцов почерка является экономически выгодным.
Чистая прибыль от реализации ПС ($ \text{П}_\text{ч} $ = \num{\netProfit} рублей) показывает экономический эффект от создания программного средства.
